...
def fixture_events(
    count: int, repository_id: str, repository_name: str
...
def _measure(
    workload: str, input_count: int, action: Callable[[], tuple[int, dict[str, Any]]]
) -> dict[str, Any]:
    gc.collect()
    tracemalloc.start()
...
def _stats_case(
    root: Path, count: int, *, detailed: bool
    ...
    return len(encoded), {
        ...
        "command_totals": sum(
            int(row.get("calls", 0)) for row in data.get("commands", [])
        )
        == expected_operations,
    }
...
def _search_case(count: int, budget: int) -> tuple[int, dict[str, Any]]:
    ...
    file_headers = sum(
        line.startswith("src/generated/file-") for line in rendered.splitlines()
    )
...
def run_benchmarks(
    root: Path, sizes: list[int], result_record_sizes: list[int], budget: int
) -> dict[str, Any]:
        ...
        for count in sizes:
            ...
            with _telemetry_paths(archive, hot):
                ...
                comparisons.append(
                    {
                        "input_count": count,
                        "default_faster_percent": (
                            round(
                                100 * (detail_wall / default_wall - 1),
                                2,
                            )
                            if default_wall
                            else 0.0
                        ),
                        "default_wall_reduction_percent": (
                            round(
                                100 * (detail_wall - default_wall) / detail_wall,
                                2,
                            )
                            if detail_wall
                            else 0.0
...
def main() -> int:
    ...
    parser.add_argument(
        "--sizes", nargs="+", type=_parse_count, default=list(EVENT_SIZES)
    )
    parser.add_argument(
        "--result-records",
        nargs="+",
        type=_parse_count,
        default=list(RESULT_RECORD_SIZES),
    )
    parser.add_argument("--budget", type=_parse_count, default=12_000)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    report = run_benchmarks(
        Path.cwd().resolve(), args.sizes, args.result_records, args.budget
    )
    if args.json:
