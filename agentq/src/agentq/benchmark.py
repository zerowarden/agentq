...
def benchmark_data(
    root: Path, commands: list[str], warmup: int, runs: int, prepare: str | None = None
) -> dict[str, Any]:
    if not commands:
...
def render_benchmark(data: dict[str, Any], *, budget: int = 0) -> str:
    ...
    lines.append(
        "Treat results as empirical measurements under the current machine/load/cache conditions, not universal performance claims."
    )
    return "\n".join(lines)
