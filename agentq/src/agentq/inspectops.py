...
class EditBundle:
    ...
    def from_wire(cls, value: dict[str, Any]) -> EditBundle:
        ...
        return cls(
            ...
            declaration=(
                ReadResult.from_wire(value["declaration"])
...
def _test_references(
    ...
    hits = tuple(hit.to_wire() for hit in result.hits if hit.role == "test")[:12]
...
def inspect_data(
    ...
    *,
    ...
    if candidate_path.exists():
        ...
        if candidate_path.is_file():
            if anchors or ranges:
                ...
                return {**wrapper, "source": source.to_wire()}
            result = {
                "kind": "file",
                "target": target,
                "path": relative,
    ...
    return {
        "kind": "lexical",
        "target": target,
        "search": lexical.to_wire(),
        "intent": intent,
        "provenance": LEXICAL,
        "coverage": lexical.coverage.to_wire(),
    }
...
def render_inspect(data: dict[str, Any], *, budget: int = 0) -> str:
    ...
    if kind == "source-windows":
        return render_read(ReadResult.from_wire(data["source"]), budget=budget)
    if kind == "ambiguous":
    ...
    if kind == "lexical":
        ...
        typed_search = SearchResult.from_wire(search)
        typed_search = replace(typed_search, coverage=typed_from_wire(visible))
    ...
    if kind in {"file", "directory"}:
        ...
        blocks = [render_outline(OutlineResult.from_wire(data["outline"]))]
        if data.get("package"):
