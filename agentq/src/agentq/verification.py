...
def _ownership_override(
    path: str, default_key: str | None, config: VerifyConfig, units: dict[str, Any]
) -> str | None:
...
def _fragment(
    ...
    *,
    ...
    return {
        ...
        "coverage": (
            coverage_block(SAMPLED, RESULT_LIMIT, STEP_LIMIT)
            if truncated
        ...
        "changed_packages": sorted(
            unit_names[key] for key in changed_keys if key in unit_names
        ),
        "dependent_packages": sorted(
            unit_names[key] for key in dependent_keys if key in unit_names
        ),
...
def _sorted_steps(
    steps: list[dict[str, Any]], order_rank: dict[str, int]
) -> list[dict[str, Any]]:
    ...
    for step in sorted(
        steps,
        key=lambda item: (
            order_rank.get(item["package_key"], 9999),
            item["priority"],
            item["kind"],
        ),
    ):
...
class NodeVerificationProvider:
    ...
    def detect(self, root: Path, repo_files: Sequence[str]) -> bool:
        return (root / "package.json").is_file() or any(
            PurePosixPath(rel).name == "package.json" for rel in repo_files
        )
    ...
    def plan_data(
        ...
        *,
        ...
        dependent_distances = (
            {}
            if depth == 0
            else transitive_dependents(effective_changed_keys, reverse, depth=depth)
        )
        ...
        for key in ordered:
            ...
            config_changed = (
                any(CONFIG_RE.search(path) for path in local_files) or scope == "global"
            )
            candidate_tests = (
                _candidate_tests(
                    owned_files,
            ...
            if scope in {"changed", "global"}:
                ...
                elif test_script and (
                    config_changed or scope == "global" or not source_files
                ):
                    steps.append(
                        _step(
                            priority=30,
                ...
                run_dependent_tests = mode == "thorough" or (
        ...
        return _fragment(
            ...
            unowned=unowned,
            changed_keys=direct_changed_keys,
            dependent_keys=dependent_distances,
            ordered=ordered,
            unit_names={key: pkg.name for key, pkg in packages.items()},
            rows=rows,
            steps=steps,
            notes=notes,
        )
...
def _manifest_units(
    repo_files: Sequence[str], manifest_name: str, root: Path
) -> list[tuple[str, Path]]:
    ...
    def plan_data(
        ...
        *,
        ...
        dependent_distances = (
            {}
            if depth == 0
            else transitive_dependents(set(direct_changed_keys), reverse, depth=depth)
        )
        ordered = dependency_order(
            set(direct_changed_keys) | set(dependent_distances), forward
        )
        order_rank = {key: index for index, key in enumerate(ordered)}
        ...
        for key in ordered:
            ...
            rows.append(
                _unit_row(
                    unit.name, key, scope, distance, local_files, limit, candidate_tests
                )
            )
            if docs_only:
                continue
            ...
            if scope == "changed":
                ...
                if tools.get("pytest") and (
                    config_changed or not source_files or not candidate_tests
                ):
                    steps.append(
                        _step(
                            priority=30,
            ...
            checker_kind = (
                "dependent-typecheck" if scope == "dependent" else "typecheck"
            )
            if tools.get("mypy"):
                steps.append(
                    _step(
                        priority=checker_priority,
                        kind=checker_kind,
                        package=unit.name,
                        package_key=key,
                        cwd=key,
        ...
        return _fragment(
            ...
            dependent_keys=dependent_distances,
            ordered=ordered,
            unit_names={key: unit.name for key, unit in units.items()},
            rows=rows,
            steps=steps,
            notes=notes,
        )
...
class CargoVerificationProvider:
    ...
    def _units(self, root: Path, repo_files: Sequence[str]) -> dict[str, PackageUnit]:
        workspace_obj = (
            _read_toml(root / "Cargo.toml") if (root / "Cargo.toml").is_file() else {}
        )
        workspace_section = (
            workspace_obj.get("workspace")
            if isinstance(workspace_obj.get("workspace"), dict)
            else {}
        )
        patterns = [
            str(item)
        ...
        for key, manifest_path in _manifest_units(repo_files, "Cargo.toml", root):
            obj = _read_toml(manifest_path)
            package = (
                obj.get("package") if isinstance(obj.get("package"), dict) else None
            )
            if package is None:
        ...
        units = {
            key: PackageUnit(
                key=key, name=str(obj["package"].get("name") or PurePosixPath(key).name)
            )
        ...
        for key, obj in manifests.items():
            ...
            resolved[key] = PackageUnit(
                key=key, name=unit.name, dependencies=frozenset(local)
            )
        return resolved
    ...
    def plan_data(
        ...
        *,
        ...
        workspace_lints = (
            workspace_section.get("lints")
            if isinstance(workspace_section.get("lints"), dict)
            else {}
        )
        ...
        for key in ordered:
            ...
            crate_lints = (
                crate_obj.get("lints")
                if isinstance(crate_obj.get("lints"), dict)
                else {}
            )
        ...
        *,
        ...
        for key in ordered:
            ...
            scope = (
                "global"
                if module_changed
                else "changed" if key in direct_changed_keys else "dependent"
            )
            distance = int(dependent_distances.get(key, 0))
            go_files = [path for path in local_files if path.endswith(".go")]
            package_dirs = sorted(
                {str(PurePosixPath(path).parent) for path in go_files}
            )
            targets = [
...
def load_verify_config(root: Path) -> VerifyConfig:
    ...
    for field, expected in (
        ...
        ("contract_patterns", "glob patterns"),
    ):
        ...
        if value is not None and (
            ...
            raise AgentQError(
                f".agentq.toml [verify] {field} must be a list of {expected}"
            )
    ...
    if ownership_raw is not None:
        ...
        for prefix, name in ownership_raw.items():
            if not isinstance(name, str):
                raise AgentQError(
                    ".agentq.toml [ownership] values must be package names"
                )
