...
from ..output_attribution import (
...
def _error_category(message: str | None) -> str | None:
    ...
    if (
        "invalid arguments" in text
        or "requires" in text
        or "provide " in text
        or "cannot be combined" in text
        or "accepts one query" in text
    ):
...
def event_metrics(
    ...
    if command in {"search", "inspect"}:
        ...
        if command == "inspect":
            ...
            semantic = (
                data.get("semantic") if isinstance(data.get("semantic"), dict) else {}
            )
    ...
    if command == "read":
        ...
        overlap = (
            data.get("read_overlap")
            if isinstance(data.get("read_overlap"), dict)
            else {}
        )
        if overlap:
            metrics["same_context_overlap_lines"] = _metric_int(
...
def read_efficiency(
    ...
    for event_index, event in enumerate(events):
        ...
        ranges = (
            metrics.get("read_ranges")
            if isinstance(metrics.get("read_ranges"), list)
            else []
        )
        if ranges:
            tracked_events += 1
...
def _command_rows(
    ...
    for event in events:
        ...
        summary["budget_removed"] += max(
            0, int(event.get("prebudget_chars", 0)) - visible
        )
    ...
    for event in ordered:
        ...
        if previous:
            if previous.get("render_budget_truncated", previous.get("truncated")):
                ...
                result["expanded_budget_retries"] += int(
                    _is_expanded_retry(previous, event)
                )
...
def _is_expanded_retry(previous: dict[str, Any], current: dict[str, Any]) -> bool:
    if not previous.get("truncated") or previous.get(
        ...
        return False
    before = (
        previous.get("expansion_controls")
        if isinstance(previous.get("expansion_controls"), dict)
        else {}
    )
    after = (
        current.get("expansion_controls")
        if isinstance(current.get("expansion_controls"), dict)
        else {}
    )
    return any(
...
def stats_presentation_model(
    ...
    profile_rows = [
        row
        for row in (data.get("output_profiles") or [])
    ...
    if cohort.get("available") and (
        ...
        detail_sections.append(
            {"name": "Comparable seven-day cohort", "rows": comparison_rows}
        )
    ...
    if read_detail_rows:
        detail_sections.append(
            {"name": "Reading contributors", "rows": read_detail_rows}
        )
...
def render_stats(
    data: dict[str, Any], *, color: str = "auto", utc: bool = False
) -> str:
    return (
