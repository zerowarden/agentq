...
def _evidence_identity(
    ...
    if key in {"preview", "signature"} and any(
        part in {"candidates", "results"} for part in ancestors
    ):
        return ("semantic", str(local_path), line, key)
    if (
        key is None
        and ancestors
        and ancestors[-1] == "lines"
        and command in {"outline", "inspect"}
    ):
...
def _text_attribution(
    command: str, data: dict[str, Any], visible: str
) -> dict[str, int]:
    result = empty_attribution()
    ...
    for fragment in sorted(fragments, key=lambda item: (-len(item), item)):
        ...
        while True:
            ...
            category = (
                "unique_evidence_chars"
                if occurrence < len(identities)
                else "duplicate_evidence_chars"
            )
            occurrence += 1
