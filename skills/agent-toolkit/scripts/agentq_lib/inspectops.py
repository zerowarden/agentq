...
def _with_metadata(result: dict[str, Any], *, intent: str, providers: list[dict[str, Any]]) -> dict[str, Any]:
    result["intent"] = intent
    result["providers"] = providers
    result["provenance"] = best_provenance(*(item["provenance"] for item in providers if item["candidate_count"])) or LEXICAL
    result["coverage"] = merge_coverage(*(item["coverage"] for item in providers))
    return result


def _test_references(root: Path, target: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    result = search_data(
        root, target, None, mode="fixed", word=True, limit=24, per_file=4,
        include_sensitive=False, view="auto", max_files=8,
    )
...
def _edit_bundle(
    ...
    *,
    ...
    if end >= start:
        try:
            bundle["declaration"] = read_data(
                root, [relative], line_ranges=[(start, min(end, start + max_lines - 1))],
                context=0, max_lines=max_lines, max_chars=260, include_sensitive=False,
                repeat=False, cache_command="inspect", budget=0, output_format="text",
            )
        except AgentQError:
    ...
    if not tests:
        verification.append("no direct test references found; verify through owning-package typecheck and tests")
    ...
    *,
    ...
    if candidate.exists():
        ...
        if candidate.is_file():
            ...
            if intent == "edit":
                ...
                result["verification"] = [
                    f"run {package['kind']} checks for {package.get('name') or package['path']} (typecheck, tests)"
                ] if package else ["no owning manifest found; verify through the workspace-level checks"]
            return result
        if anchors or ranges:
    ...
    if _IDENTIFIER_RE.fullmatch(target):
        resolution = resolve_symbol(
            root, target, paths=paths, limit=limit, lang=lang, context=context,
        ...
        if ts_candidates and py_candidates:
            return _with_metadata({
                ...
                "typescript": ts, "python": python,
            }, intent=intent, providers=providers)

        if ts_candidates:
            return _finish_symbol_result(
                root, {"kind": "semantic", "target": target, "semantic": ts},
                target=target, intent=intent, providers=providers,
                navigation=ts, candidate=_ts_candidate(ts), max_lines=max_lines,
            )
        if python:
            return _finish_symbol_result(
                root, {"kind": "python", "target": target, "python": python},
                target=target, intent=intent, providers=providers,
                navigation=python, candidate=py_candidates[0] if py_candidates else None,
                max_lines=max_lines,
            )

        lexical = resolution.fallback.result if resolution.fallback else search_data(
            root, target, paths,
        ...
        return _with_metadata({
            "kind": "lexical", "target": target, "search": lexical,
        }, intent=intent, providers=providers)
    ...
    return {
        "kind": "lexical", "target": target, "search": lexical, "intent": intent,
        "provenance": LEXICAL, "coverage": lexical.get("coverage") or complete_coverage(),
    }
...
def _ts_candidate(ts: dict[str, Any]) -> dict[str, Any] | None:
    ...
    results = definitions.get("results") if isinstance(definitions.get("results"), list) else []
    if not results:
        return None
    span = ts.get("declaration_span") if isinstance(ts.get("declaration_span"), dict) else {}
    return {
...
def _finish_symbol_result(
    ...
    *,
    ...
    if intent == "edit" and candidate:
        bundle = _edit_bundle(
            root, target, navigation=navigation, candidate=candidate, max_lines=max_lines,
        )
        result["kind"] = "edit"
        result["edit"] = bundle
...
def render_inspect(data: dict[str, Any], *, budget: int = 0) -> str:
    ...
    if kind in {"file", "directory"}:
        ...
        rendered, _ = budget_text_records(
            header, blocks, budget, separator="\n",
...
def _render_ambiguous(data: dict[str, Any], *, budget: int) -> str:
    ...
    for item in _candidates(ts or None):
        ...
        records.append(f"  typescript {item['path']}:{item['line']}:{item['column']}{detail}")
    ...
    for item in _candidates(python or None):
        ...
        records.append(f"  python {item['file']}:{item['line']} [{item['kind']}] {item['signature']}{scope}")
    header = (
...
def _render_edit(data: dict[str, Any], *, budget: int) -> str:
    ...
    if tests:
        blocks.append("related tests:\n" + "\n".join(
            f"  {h['path']}:{h['line']} {h['text']}" for h in tests
        ))
    package = data.get("package")
    if package:
        blocks.append(f"owning package: {package.get('name') or package['path']} ({package['path']})")
    blocks.extend(f"verify: {item}" for item in data.get("verification") or [])
    rendered, _ = budget_text_records(
        header, blocks, budget, separator="\n\n",
