...
from .common import AgentQError, is_sensitive_path, scope_match
...
from .runtime import context_cache_dir, secure_dir
...
def _failure_result(
    path: Path, restored: list[str], failed: list[str]
) -> CommitResult:
    if failed:
        return CommitResult(
