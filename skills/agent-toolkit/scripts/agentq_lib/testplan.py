...
from .workspace import changed_files, is_public_contract_change, matches_pattern, package_manager
...
def test_plan_data(
    ...
    *,
    ...
    if config.ignore and changed:
        kept: list[str] = []
        for path in changed:
            if any(matches_pattern(path.strip("/"), pattern) for pattern in config.ignore):
                ignored.append(path)
            else:
                kept.append(path)
        changed = kept
    ...
    contract_changed = any(
        ...
        or any(matches_pattern(path.strip("/"), pattern) for pattern in config.contract_patterns)
        for path in changed
    )

    repo_files = list_repo_files(root)
    selected = [
        provider for provider in VERIFICATION_PROVIDERS
    ...
    if ignored:
        ...
        orchestrator_notes.append(f"{len(ignored)} changed file(s) ignored via .agentq.toml [verify] ignore: {summary}")
