...
def _resolve_absolute(root: Path, value: str | Path, *, allow_outside: bool = False) -> Path:
...
def resolve_repo_path(root: Path, value: str | Path, *, must_exist: bool = False) -> RepoPath:
    ...
    relative = "." if absolute == root_abs else absolute.relative_to(root_abs).as_posix()
