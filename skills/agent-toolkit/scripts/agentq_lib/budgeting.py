...
_MISSING = object()
_PREFERRED_LISTS = (
    "items", "results", "hits", "files", "hunks", "steps", "matches",
    "candidates", "packages", "recent", "commands",
)
...
class RenderedText(str):
    ...
    def __new__(
        ...
        *,
        ...
        instance.prebudget_chars = len(value) if prebudget_chars is None else max(len(value), prebudget_chars)
...
def _project(value: Any, path: str, budget: int, omitted: dict[str, int]) -> tuple[Any, int] | object:
    if budget < 2:
    ...
    if isinstance(value, dict):
        ...
        used = 2
        ordinary = [key for key, item in value.items() if not isinstance(item, list) and key != "_agentq"]
        list_keys = [key for key in _PREFERRED_LISTS if isinstance(value.get(key), list)]
        list_keys.extend(key for key, item in value.items() if isinstance(item, list) and key not in list_keys)
        for key in [*ordinary, *list_keys]:
            ...
            child = _project(value[key], child_path, budget - used - fixed_cost, omitted)
            if child is _MISSING:
                continue
            child_value, child_size = child
            if used + fixed_cost + child_size > budget:
                _record_omission(omitted, child_path, len(value[key]) if isinstance(value[key], list) else 1)
                continue
...
def project_json(data: dict[str, Any], budget: int) -> tuple[str, bool]:
    ...
    for _ in range(2):
        ...
        projected = result[0] if result is not _MISSING and isinstance(result[0], dict) else {}
        projected["_agentq"] = {
            "truncated": True,
...
def budget_text_records(
    ...
    *,
    ...
    for raw_record in records:
        ...
        reserve = len(separator) + len(marker(remaining_after)) if remaining_after else 0
    ...
    return rendered_text(visible, prebudget_chars=full_chars, truncated=truncated), truncated
