...
AGENTQ = SCRIPTS / "agentq"

from agentq_lib import context_cache as cache_module  # noqa: E402
from agentq_lib import state as state_module  # noqa: E402
...
class CollectorWritesNothingTests(DeliveryHarness):
    def test_collector_only_and_nested_calls_write_no_receipts(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            search_module.read_data(self.repo, ["pkg/mod.py:1-3"])
...
class EmissionRecordingTests(DeliveryHarness):
    ...
    def test_partial_window_records_only_emitted_fragments(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            data = search_module.read_data(
                self.repo, ["pkg/mod.py:1-10"], budget=100000, output_format="text"
            )
            # Simulate a truncated final render carrying only lines 1-5.
            narrow = dict(data)
            item = dict(data["items"][0])
            item["lines"] = item["lines"][:5]
            item["end"] = 5
            narrow["items"] = [item]
            visible = str(search_module.render_read(narrow))
            evidence, rows = cache_module.extract_delivered_fragments(
                self.repo, "read", narrow, visible, "text"
            )
    ...
    def test_variant_and_version_changes_are_not_suppressed(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            narrow = search_module.read_data(
                self.repo,
                ["pkg/mod.py:1-3"],
                max_chars=40,
                budget=100000,
            ...
            version = narrow["items"][0]["version"]
    ...
    def test_failed_write_flush_and_partial_create_no_receipt(self) -> None:
        from agentq_lib.cli.emit import emit
            ...
            for mode in ("write", "flush", "partial"):
                with mock.patch.object(sys, "stdout", FailingStdout(mode)):
                    with self.assertRaises(BrokenPipeError):
                        emit(
                            args,
                            read_data(self.repo, ["pkg/mod.py:1-2"]),
                            render_read,
                            root=self.repo,
                        )
            self.assertEqual(self.ledger_counts(), (0, 0))
    ...
    def test_unpersisted_emission_reports_no_receipt(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            ...
            with mock.patch("builtins.print"):
                ...
                unmigrated = emit(
                    args, read_data(self.repo, ["pkg/mod.py:1-2"]), render_read
                )
                ...
                empty = emit(
                    args,
                    read_data(self.repo, ["pkg/empty.py"]),
                    render_read,
                    root=self.repo,
                )
    ...
    def test_consumer_session_and_epoch_isolation(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            data = search_module.read_data(
                self.repo, ["pkg/mod.py:1-3"], budget=100000, output_format="text"
            )
            visible = str(search_module.render_read(data))
            _, rows = cache_module.extract_delivered_fragments(
                self.repo, "read", data, visible, "text"
            )
            identity = cache_module.suppression_identity(self.repo)
