...
class TextManifestTests(ManifestHarness):
    def test_manifest_matches_final_bytes_including_newline_and_encoding(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            data = read_data(self.repo, ["doc.txt:1-4"])
            ...
            with mock.patch.object(sys, "stdout", capture):
                dispatch = emit(args, data, render_read, root=self.repo)
    ...
    def test_projection_drops_only_unmanifested_records(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            data = search_module.read_data(
                self.repo,
                ["doc.txt:1-10", "doc.txt:21-30"],
                budget=100000,
                output_format="json",
            )
            self.assertEqual(len(data["items"]), 2)
            full = json.dumps(data, ensure_ascii=False, separators=(",", ":"))
            from agentq_lib.budgeting import project_json
...
class BudgetRecoveryTests(ManifestHarness):
    def test_zero_fit_budget_returns_larger_recovery_budget(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env, clear=False):
            data = search_module.read_data(
                self.repo, ["doc.txt:1-30"], budget=10, output_format="text"
            )
    ...
    def test_tiny_budget_is_an_explicit_error_not_an_empty_page(self) -> None:
        for output_format in ("text", "json"):
            result = self.aq(
                "read", "doc.txt:1-30", "--format", output_format, "--budget", "10"
            )
    ...
    def test_recovery_budget_still_renders_a_continuation(self) -> None:
        result = self.aq("read", "doc.txt:1-30", "--format", "text", "--budget", "200")
