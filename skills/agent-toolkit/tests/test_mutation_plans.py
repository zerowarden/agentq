...
from mutation_fixture import MutationSafetyTestCase, digest  # noqa: E402
...
class CommitSafetyTests(MutationSafetyTestCase):
    ...
    def test_external_edit_before_rollback_is_preserved(self) -> None:
        ...
        with mock.patch("agentq_lib.mutation_apply.os.replace", raced_replace):
            with self.assertRaises(mutation_apply.MutationApplyError) as caught:
                codemod.apply_data(
                    self.repo,
                    None,
    ...
    def test_cancellation_during_commit_rolls_back(self) -> None:
        ...
        with mock.patch("agentq_lib.mutation_apply.os.replace", cancelling_replace):
        ...
        for mode in ("fixed", "regex"):
            with self.subTest(mode=mode):
                ...
                loaded_dry = codemod.apply_data(
                    repo, None, None, scopes=["."], mode=None, apply=False,
                    plan=plan_file,
                )
                ...
                loaded = codemod.apply_data(
                    repo, None, None, scopes=["."], mode=None, apply=True,
                    plan=plan_file,
                )
