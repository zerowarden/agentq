...
class AgentQIntegrationTest(unittest.TestCase):
    ...
    def test_output_attribution_partitions_json_and_text_exactly(self) -> None:
        with mock.patch.object(sys, "path", [str(AGENTQ.parent), *sys.path]):
            from agentq_lib.output_attribution import (
                attribute_output,
                attribution_total,
            )
    ...
    def test_online_read_and_diff_advice_never_load_historical_telemetry(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.env):
            with mock.patch.object(
                ...
                read = search_module.read_data(
                    self.repo, ["packages/a/src/index.ts:1-3"]
                )
    ...
    def test_phase_four_workflow_replays_bound_calls_output_and_evidence(self) -> None:
        ...
        with mock.patch(
            "agentq_lib.navigation.ts_nav_data", return_value=overview
    ...
    def test_ctags_signatures_are_qualified_with_symbol_names(self) -> None:
        ...
        with mock.patch.object(
            search_module, "find_executable", return_value="/fake/ctags"
        ):
            with mock.patch.object(
                search_module, "list_repo_files", return_value=["fixture.py"]
            ):
                with mock.patch.object(
                    search_module, "run_cmd", return_value=completed
                ):
                    outline = search_module._outline_ctags(
                        self.repo, ["."], None, False, 20
                    )
        self.assertEqual(
            outline["symbols"][0]["signature"], "build(value, *, strict=False)"
        )
