...
from agentq_lib.paths import RepoPath, RepoScope, resolve_repo_path, resolve_repo_scopes  # noqa: E402
...
class PathConfinementTests(unittest.TestCase):
    ...
    def test_impact_refuses_outside_manifest(self) -> None:
        ...
        out = self._run(["impact", "../outside/package.json", "--repo", str(self.repo)], env=env)
        self.assertNotEqual(out.returncode, 0)
        self.assertIn("outside repository", out.stdout + out.stderr)

    def _run(self, args, env=None):
        return __import__("subprocess").run(
            [sys.executable, AGENTQ, *args], capture_output=True, text=True,
            env=env or os.environ, timeout=60,
        )
...
class RegexEquivalenceTests(unittest.TestCase):
    ...
    def test_scan_apply_match_sets_equal(self) -> None:
        ...
        result = codemod.apply_data(
            self.repo, "foo", "QUX", mode="regex", scopes=["."], apply=True,
        )
...
class CodemodPlanTests(unittest.TestCase):
    ...
    def test_plan_id_deterministic(self) -> None:
        p1 = codemod.build_codemod_plan(self.repo, "foo", "X", "regex", None, ["."], False)
        p2 = codemod.build_codemod_plan(self.repo, "foo", "X", "regex", None, ["."], False)
        self.assertEqual(p1["plan_id"], p2["plan_id"])

    def test_stale_plan_rejected(self) -> None:
        plan = codemod.build_codemod_plan(self.repo, "foo", "X", "fixed", None, ["."], False)
        ...
        with self.assertRaises(codemod.AgentQError):
            codemod.apply_plan(self.repo, plan, apply=True, max_files=100, include_sensitive=False)
    ...
    def test_apply_from_plan_succeeds(self) -> None:
        plan = codemod.build_codemod_plan(self.repo, "foo", "Z", "regex", None, ["."], False)
        result = codemod.apply_plan(self.repo, plan, apply=True, max_files=100, include_sensitive=False)
...
class SensitiveExclusionTests(unittest.TestCase):
    ...
    def test_sensitive_excluded_by_default(self) -> None:
        result = codemod.apply_data(
            self.repo, "secret123", "REDACTED", mode="fixed", scopes=["."], apply=True,
        )
...
class EvidenceVocabularyTests(unittest.TestCase):
    ...
    def test_downgrade_never_upgrades_status(self) -> None:
        ...
        self.assertEqual(evidence.downgrade(current, evidence.COMPLETE)["status"], evidence.PARTIAL)
        self.assertEqual(evidence.downgrade(current, evidence.UNKNOWN)["status"], evidence.UNKNOWN)
        self.assertEqual(
            evidence.downgrade(current, evidence.PARTIAL, evidence.PROVIDER_UNAVAILABLE)["reason"],
    ...
    def test_status_of_accepts_blocks_legacy_strings_and_garbage(self) -> None:
        ...
        self.assertEqual(evidence.status_of({"status": "sampled", "reason": ["result_limit"]}), evidence.SAMPLED)
    ...
    def test_best_provenance_prefers_strongest_evidence(self) -> None:
        self.assertEqual(evidence.best_provenance("lexical", "semantic", "syntactic"), "semantic")
